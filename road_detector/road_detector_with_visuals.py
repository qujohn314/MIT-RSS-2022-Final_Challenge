...
class RoadDetector:
    ...
    def callback(self, img_msg):
        ...
        rgb_img_og = np.array(rgb_img_tensor).transpose(1,2,0)
        #print(rgb_img_og.shape[0], rgb_img_og.shape[1], rgb_img_og.shape[2])
        if self.count%4 == 0:
            print("Displaying image")
            np.savetxt('np_img.txt', rgb_img.reshape(rgb_img_og.shape[0],-1))
            np.savetxt('mask.txt', mask)
            os.system("python3 visualize_image.py")
